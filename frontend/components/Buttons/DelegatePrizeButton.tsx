...
interface DelegatePrizeButtonProps {
...
const DelegatePrizeButton = ({
  listen,
  contractConfig,
  setLoading,
  setMinted,
  setEventData,
}: DelegatePrizeButtonProps) => {
  //Claim Prize
  const { config: collectConfig, data: dataCollect } = usePrepareContractWrite({
    ...contractConfig,
    functionName: "letsGoFishing",
    args: ['vaultaddress'], //pass in the address of the vault
    overrides: {
      gasLimit: 1500000,
    },
    onError(error: any) {
